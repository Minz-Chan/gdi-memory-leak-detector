...
void xmloperator::ShowHandle(string Handle, string &Value)
{
	...
	xml->IntoElem(); 
	Tag =xml->GetAttrib("Value");
	if (strcmp(Tag.c_str(),Handle.c_str())==0)
	{
		Find_Data( Tag, Val);
	}
...
string xmloperator::Show_Data(PHANDLE_INFORMATION p)
{
	...
	s=s+"Registers :\r\n";
	sprintf(t, "!@Eax %u  Ebp %u  Ebx %u \r\n!@Ecx %u  Edi %u  Edx %u \r\n!@EFlags %u  Eip %u  Esi %u \r\n!@Esp %u  SegCs %u  SegSs %u", p->registers.Eax,p->registers.Ebp,
		p->registers.Ebx,p->registers.Ecx,p->registers.Edi,p->registers.Edx,p->registers.EFlags,p->registers.Eip,p->registers.Esi,
		p->registers.Esp,p->registers.SegCs,p->registers.SegSs);
	s=s+t+"\r\nCallStack  : \r\n";
...
void xmloperator::SetFileName(string filename){
