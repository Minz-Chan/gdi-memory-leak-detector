...
BOOL StackWalker::GetCallstack(PHANDLE_INFORMATION pHandleInfo, HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
  CONTEXT c;;
  ...
  // Get info of registers
  pHandleInfo->registers.Edi = c.Edi;
  pHandleInfo->registers.Esi = c.Esi;
  pHandleInfo->registers.Eax = c.Eax;
  pHandleInfo->registers.Ebx = c.Ebx;
  pHandleInfo->registers.Ecx = c.Ecx;
  pHandleInfo->registers.Edx = c.Edx;
  pHandleInfo->registers.Eip = c.Eip;
  pHandleInfo->registers.Ebp = c.Ebp;
  pHandleInfo->registers.SegCs = c.SegCs;
  pHandleInfo->registers.EFlags = c.EFlags;
  pHandleInfo->registers.Esp = c.Esp;
  pHandleInfo->registers.SegSs = c.SegSs;
  ...
  for (frameNum = 0; ; ++frameNum )
  {
    ...
    if (s.AddrPC.Offset != 0)
    {
      ...
      { // got module info OK
		...
		MODULE_INFORMATION moduleInfo = {0};
		moduleInfo.BaseOfImage = Module.BaseOfImage;
		moduleInfo.CheckSum = Module.CheckSum;
		moduleInfo.ImageSize = Module.ImageSize;
		moduleInfo.TimeDateStamp = Module.TimeDateStamp;
		moduleInfo.FileVersion = m_fileVersion;
		strcpy_s(moduleInfo.ImageName, Module.ImageName);
		strcpy_s(moduleInfo.LoadedImageName, Module.LoadedImageName);
		strcpy_s(moduleInfo.ModuleName, Module.ModuleName);

		// save the related information of module
		(pHandleInfo->moduleInfo).push_back(moduleInfo);
    ...
    if (frameNum == 0)
      et = firstEntry;
    this->OnCallstackEntry(pHandleInfo, et, csEntry);
    
    if (s.AddrReturn.Offset == 0)
    {
      this->OnCallstackEntry(pHandleInfo, lastEntry, csEntry);
...
void StackWalker::OnCallstackEntry(PHANDLE_INFORMATION pHandleInfo, CallstackEntryType eType, CallstackEntry &entry)
{
  CHAR buffer[STACKWALK_MAX_NAMELEN];
  if ( (eType != lastEntry) && (entry.offset != 0) )
  {
    ...
    if (entry.lineFileName[0] == 0)
    {
      ...
      _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "[%p, %p] %s --> %s\n", (LPVOID) entry.baseOfImage, (LPVOID) entry.offset, entry.moduleName, entry.name);
	   
    }
    else
      _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%s (%d): %s\n", entry.lineFileName, entry.lineNumber, entry.name);
    //OnOutput(buffer);
	pHandleInfo->callStack.callStack.push_back(buffer);
